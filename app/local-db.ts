...
export type Expense = { id: number; payerPersonId: number; payerName: string; title: string; amount: number; expenseDate: string };
...
type StoredExpense = Omit<Expense, "id" | "payerName"> & { id?: number; groupId: number; createdAt: string };
  ...
  const groups = storedGroups
    ...
    .map((group) => {
      ...
      const expenses = storedExpenses
        ...
        .map((expense) => ({ id: expense.id, payerPersonId: expense.payerPersonId, payerName: names.get(expense.payerPersonId) ?? "نامشخص", title: expense.title, amount: expense.amount, expenseDate: expense.expenseDate }))
...
export async function applyFinanceOperation(payload: Record<string, unknown>) {
  ...
  if (operation === "add_expense") {
    ...
    const allocations = members.map((member, index) => {
      const shareAmount = index === members.length - 1 ? amount - allocated : Math.floor((amount * member.shareWeight) / totalWeight);
